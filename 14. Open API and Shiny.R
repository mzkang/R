...
# list 형태의 item_list를 data frome 형태로 저장
item_list = lapply(item_list, function(x) return(xml_text(xml_children(x))))
item_list[[1]]
item_list[[2]]

item_dat = do.call('rbind',item_list)  # do.call : 재귀 호출
 # for (i in 1:끝)
 # {a <- rind(a,item_list[[i]])}
...
ui = fluidPage(          # html 코드로 작성해주는 함수들
...
ui = fluidPage(
  ...
  sidebarPanel(
    selectInput('xcol', 'X Variable', names(iris)), # 실제 이름은 'xcol'인데 
    selectInput('ycol', 'Y Variable', names(iris),  # 'X variable'로 보임
...
server = function(input, output)
{
  selectedData = reactive({   # selectedData는 df가 아니라 함수임. 지정.
    iris[, c(input$xcol, input$ycol)]
  })
  clusters <- reactive({
    kmeans(selectedData(), input$clusters) # selectedData 함수라서 ()
  })
...
shinyApp(ui = ui, server = server)
  ...
  # 샤이니 안 쓰고 해보기
names(iris)
selectedData <- iris[,c(1,3)]  # selectedData는 df
clusters <- kmeans(selectedData, 5)
plot(selectedData, 
