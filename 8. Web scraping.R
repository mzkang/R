...
# ------------------------
html_tvcast %>% html_nodes(".title a") %>% head(n=3)  # title아래 a구조만 찾겠다
                                        # title이후 띄어쓰기는 tree구조를 나타냄
# -----------------------
html_tvcast %>% html_nodes(".title a") %>% html_text()%>%head()
                                    #html_text()는 태그를 다 떼고 텍스트만 가져옴
# -----------------------
...
# -----------------------
url = "http://www.weather.go.kr/weather/observation/currentweather.jsp?auto_man=m&type=t99&tm=2018.07.10.17%3A00&x=19&y=3"
...
tmp <- webpage %>% html_nodes("table.table_develop3") %>% 
  html_table(header = FALSE, fill=TRUE)%>%   # fill=TRUE : table 안 깨지게
  data.frame()
head(tmp)

# -----------------------
 #1
Sys.setlocale("LC_ALL", "Korean")   # encoding 변경 
head(tmp)
 
 #2
for(i in 1:ncol(tmp)){
  tmp[,i] = rvest::repair_encoding(tmp[,i])       # 최적의 encoding 자동 변경
}                                                 # 안 될 때도 있음 ^^
head(tmp)
